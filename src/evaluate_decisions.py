...
def load_predictions(results_dir: str) -> pd.DataFrame:
    rows = []
    for path in sorted(Path(results_dir).glob("*.json")):
        data = load_json(path)
        jd_path = data["jd_path"]
        job_file = Path(jd_path).name
...
def evaluate(results_dir: str, label_path: str, output_path: str) -> None:
...
if __name__ == "__main__":
    evaluate(
        results_dir="results/batch_decisions",
        label_path="data/labels/job_labels.csv",
        output_path="results/evaluation_metrics.csv"
    )